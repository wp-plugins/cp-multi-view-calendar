...
; (function($) {
    ...
    $.fn.bcalendar = function(option) {
        ...
        function render() {
            ...
            switch (option.view) {
                ...
                case "nDays":
                    ...
                    gridcontainer.css("overflow-y", "visible").height(option.height - 8);//overflow-x: hidden;    overflow-y: scroll;
                    break;
                case "nMonth":
                    BuildYearView(showday, events, config);
                    //gridcontainer.css("overflow-y", "visible").height($('#nmonths'+option.thecontainer).height());
            ...
            {
                if (option.view=="week" || option.view=="day" || option.view=="nDays")
                    gridcontainer.css("overflow-y", "auto");
                for (var i=0;i<option.rowsList.length;i++)
                    ...
                    $("#weekViewAllDaywk"+option.thecontainer+i).height($("#weekViewAllDaywk"+option.thecontainer+i).parent().height());
            }        
            ...
            var mydatepicker = $( "#nmonths"+option.thecontainer ).datepicker({numberOfMonths: option.numberOfMonths,firstDay:option.weekstartday,defaultDate:showday,showOtherMonths: true,
             ...
             function showDialogNMonth(dates,item,idover)
             {
                 ...
                 {                                                                                                                                                                                                                                             
                     ...
                     $("#bbit-cs-buddle").dialog("close");
                     $(".mv_dlg_nmonth").dialog("close");
                     if (showTitle)
             ...
             if (option.showtooltip || option.readonly != true)
             {
                 ...
                 {
                     $("#nmonths"+option.thecontainer+" .ui-state-active").live('mouseover', function(){
        ...
        function BuildDaysAndWeekView(startday, l, events, config) {
            ...
            {
                html.push("<tr><th width=\"70\" now>&nbsp;</th>");
                BuildWTHeader(html, days);
                html.push("<th width=\"16\">&nbsp;</th></tr>");
            }
            if (option.rowsList=="" || (option.dayWithTime && option.view=="day"))
        ...
        function parseED(data) {
            if (data.length > 6) {
                var e = [];
                e.push(data[0], data[1], new Date(data[2]), new Date(data[3]), parseInt(data[4]), parseInt(data[5]), parseInt(data[6]), data[7] != undefined ? parseInt(data[7]) : -1, data[8] != undefined ? parseInt(data[8]) : 0);
